...
![The discourse ladder: prompt, context, loop, graph engineering — all on the write path, ending at "code exists… and then what?"](/images/after-the-loop/diagram-ladder.svg "fig 1 · the discourse ladder, all of it on the write path")
...
![Self-grading: agent writes code, writes tests from that code, suite goes green, merge. Oracle-derived: spec transcribed independently, corpus of expected outputs, diffed against the implementation](/images/after-the-loop/diagram-selfgrading.svg "fig 4 · self-grading tests vs an independent oracle")
...
![The outer loop: MVP, derive oracles, agents develop, mutation gate loops survivors back, comprehension brake halts until a human can explain the change, CI review diffs against the oracle, merge](/images/after-the-loop/diagram-outerloop.svg "fig 5 · the outer loop with verification gates")
